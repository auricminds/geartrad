...
import { Heart, ShoppingCart, Star, Shield, Zap, Trophy, Check } from 'lucide-react';
    ...
    <Link href={`/${locale}/listing/${listing.id}`}>
      <div
        className={cn(
          ...
          'hover:border-purple/40 transition-all duration-300',
          'hover:shadow-xl',
      ...
      >
        ...
        <div className="relative overflow-hidden aspect-[4/3]">
          ...
          <button
            ...
            onClick={handleLike}
            className={cn(
              'absolute top-3 end-3 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200',
              liked
          ...
          {listing.rank && (
            <div className="absolute bottom-3 start-3">
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/70">
                <Trophy className={cn('w-3.5 h-3.5', rankColors[listing.rank])} />
                <span className={cn('text-xs font-bold', rankColors[listing.rank])}>
                  {listing.rank}
                </span>
              </div>
            </div>
